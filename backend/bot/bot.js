...
class Bot extends Agent {
  ...
  constructor(
    config,
    initialState = "ONLINE",
    subscribeAllConversations = false
  ) {
    // Set agent config from environment or file (env takes precedence)
    const _config = {
      accountId:
        process.env.LP_ACCOUNTID || process.env.LP_ACCOUNT || config.accountId,
      username:
        process.env.LP_USERNAME || process.env.LP_USER || config.username,
  ...
  init() {
    ...
    this.on("connected", (message) => {
      ...
      if (!this.subscribeAllConversations)
        convSubParams.agentIds = [this.agentId];
      this.subscribeExConversations(convSubParams, (e, resp) => {
        if (e) {
          log.error(`[bot.js] subscribeExConversations: ${JSON.stringify(e)}`);
        } else {
          log.info(
            `[bot.js] subscribeExConversations: ${JSON.stringify(resp)}`
          );
        }
      });
    ...
    this.on("routing.RoutingTaskNotification", (body) => {
      log.info(
        `[bot.js] routing.RoutingTaskNotification: ${JSON.stringify(body)}`
      );
      body.changes.forEach((change, index) => {
        log.silly(
          `[bot.js] routing.RoutingTaskNotification change ${index}: ${JSON.stringify(
            change
          )}`
        );
      });
    ...
    this.on("routing.AgentStateNotification", (body) => {
      log.info(
        `[bot.js] routing.AgentStateNotification: ${JSON.stringify(body)}`
      );
      body.changes.forEach((change, index) => {
        log.silly(
          `[bot.js] routing.AgentStateNotification change ${index}: ${JSON.stringify(
            change
          )}`
        );
      });
    ...
    this.on("cqm.ExConversationChangeNotification", (body) => {
      log.info(
        `[bot.js] cqm.ExConversationChangeNotification: ${JSON.stringify(body)}`
      );
      body.changes.forEach((change, index) => {
        log.silly(
          `[bot.js] cqm.ExConversationChangeNotification change ${index}: ${
            JSON.stringify(change.result.event) || "[no event]"
        ...
        if (change.type === "UPSERT") {
          ...
          if (!this._isInMyConversations(change.result.convId)) {
            ...
            log.silly(
              `[bot.js] ${
                change.result.convId
              } added to myConversations: ${JSON.stringify(
                this.myConversations
              )}`
            );
            ...
            this.myConversations[
              change.result.convId
            ].consumerProfile = this.getConsumerProfile(
              change.result.convId,
              change.result.conversationDetails
            );
            ...
            this.subscribeMessagingEvents(
              ...
              (e) => {
                if (e) {
                  log.error(
                    `[bot.js] subscribeMessagingEvents: ${JSON.stringify(e)}`
                  );
                } else {
          ...
          log.silly(
            `[bot.js] ${
              change.result.convId
            } removed from myConversations: ${JSON.stringify(
              this.myConversations
            )}`
          );
        }
    ...
    this.on("ms.MessagingEventNotification", (body) => {
      ...
      if (
        !body.changes.find((change) => {
          return change.event.type === "ChatStateEvent";
        })
      ) {
        log.info(
          `[bot.js] ms.MessagingEventNotification: ${JSON.stringify(body)}`
        );
      } else {
        log.silly(
          `[bot.js] ms.MessagingEventNotification: ${JSON.stringify(body)}`
      ...
      body.changes.forEach((change) => {
        log.silly(
          `[bot.js] ms.MessagingEventNotification: ${JSON.stringify(
            change.event
          )} | ${JSON.stringify(change)}`
        );

        if (this._isInMyConversations(change.dialogId)) {
          // This check is necessary because of the subscription bug
          // add to respond list all content events not by me
          if (
            change.event.type === "ContentEvent" &&
            change.originatorId !== this.agentId
          ) {
          ...
          if (
            change.event.type === "AcceptStatusEvent" &&
            change.originatorId === this.agentId
          ) {
      ...
      if (this.myConversations[body.dialogId]) {
        Object.keys(respond).forEach((key) => {
          ...
          if (
            this.getRole(
              this.myConversations[body.dialogId].conversationDetails
            ) === "ASSIGNED_AGENT"
          ) {
    ...
    this.on("notification", (msg) => {
      if (!handledMessageTypes.includes(msg.type)) {
        log.error(
          `[bot.js] Got an unhandled message: ${msg.type} ${JSON.stringify(
            msg
          )}`
        );
      }
  ...
  getConsumerProfile(conversationId, conversationDetails) {
    const consumerId = conversationDetails.participants.filter(
      (p) => p.role === "CONSUMER"
    )[0].id;
    this.getUserProfile(consumerId, (e, resp) => {
      if (e) log.error(`[bot.js] getConsumerProfile: ${JSON.stringify(e)}`);
        ...
        log.info(
          `[bot.js] getConsumerProfile: conversation ${conversationId} consumerProfile ${JSON.stringify(
            resp
          )}`
        );
      }
          ...
          if (ring.ringState === "WAITING") {
            this.updateRingState(
              ...
              (e, resp) => {
                if (e) {
                  log.error(
                    `[bot.js] acceptWaitingConversations ${JSON.stringify(e)}`
                  );
                } else {
                  log.info(
                    `[bot.js] acceptWaitingConversations: Joined conversation ${JSON.stringify(
                      change.result.conversationId
                    )}, ${JSON.stringify(resp)}`
                  );
                }
              }
  ...
  joinConversation(conversationId, role, announce) {
    if (!/^(READER|MANAGER|ASSIGNED_AGENT)$/.test(role)) {
      return false;
    }
    this.updateConversationField(
      ...
      (e, resp) => {
        if (e) {
          log.error(`[bot.js] joinConversation ${JSON.stringify(e)}`);
        } else {
          log.info(
            `[bot.js] joinConversation: Joined conversation ${JSON.stringify(
              conversationId
            )}, ${JSON.stringify(resp)}`
          );
  ...
  sendText(conversationId, message) {
    log.silly(
      `[bot.js] sending text ${message} to conversation ${conversationId}`
    );
    this.publishEvent(
      {
        dialogId: conversationId,
      ...
      (e, r) => {
        if (e) {
          log.error(`[bot.js] sendText ${message} ${JSON.stringify(e)}`);
        } else {
          log.silly(`[bot.js] sendText successful: ${JSON.stringify(r)}`);
        }
      }
    );
  }
  ...
  sendRichContent(conversationId, card) {
    log.silly(
      `[bot.js] sending structured content card ${card.id} to conversation ${conversationId}`
    );
    this.publishEvent(
      {
        dialogId: conversationId,
      ...
      (e, r) => {
        if (e) {
          log.error(
            `[bot.js] sendRichContent card ${card.id} ${JSON.stringify(
              e
            )} ${JSON.stringify(card.content)}`
          );
        } else {
          log.silly(
            `[bot.js] sendRichContent successful: ${JSON.stringify(r)}`
          );
        }
      }
    );
  ...
  markAsRead(conversationId, sequenceArray) {
    this.publishEvent(
      ...
      (e) => {
        if (e) {
          log.error(`[bot.js] markAsRead ${JSON.stringify(e)}`);
        } else {
          log.silly("[bot.js] markAsRead successful");
        }
      }
    );
  }
  ...
  transferConversation(conversationId, targetSkillId) {
    log.info(
      `[bot.js] transferring conversation ${conversationId} to skill ${targetSkillId}`
    );
    this.updateConversationField(
      {
        conversationId: conversationId,
      ...
      (e, r) => {
        if (e) {
          log.error(`[bot.js] transferConversation ${JSON.stringify(e)}`);
        } else {
          log.silly(
            `[bot.js] transferConversation successful ${JSON.stringify(r)}`
          );
        }
      }
    );
  ...
  removeParticipant(conversationId, role) {
    ...
    this.updateConversationField(
      ...
      (e) => {
        if (e) {
          log.error(`[bot.js] removeParticipant ${JSON.stringify(e)}`);
        } else {
          log.silly("[bot.js] removeParticipant successful");
        }
      }
    );
  }
  ...
  closeConversation(conversationId) {
    this.updateConversationField(
      ...
      (e) => {
        if (e) {
          log.error(`[bot.js] closeConversation ${JSON.stringify(e)}`);
        } else {
          log.silly("[bot.js] closeConversation successful");
        }
      }
    );
  }
  ...
  getRole(conversationDetails, pid = this.agentId) {
    let participant = conversationDetails.participants.filter(
      (p) => p.id === pid
    )[0];
  ...
  _updateMyConversation(conversationId, result) {
    ...
    if (result.lastContentEventNotification) {
      _newData.lastContentEventNotification = {
        ...
        originatorMetadata: {
          role: result.lastContentEventNotification.originatorMetadata.role,
        },
  ...
  _reconnect(delay = botConfig.reconnectInterval, attempt = 1) {
    let _this = this;
    log.warn(
      `[bot.js] reconnecting in ${Math.round(delay)}s (attempt ${attempt} of ${
        botConfig.reconnectAttempts
      })`
    );
    this._retryConnection = setTimeout(() => {
      _this.reconnect();
      if (++attempt <= botConfig.reconnectAttempts) {
        _this._reconnect(delay * botConfig.reconnectRatio, attempt);
      } else {
        log.error`[bot.js] failed to reconnect after ${botConfig.reconnectAttempts} attempts`;
      }
    }, delay * 1000);
  }
}
...
const getClock = (context) => {
  ...
  context.getClock({}, (e, resp) => {
    if (e) {
      log.error(`[bot.js] getClock: ${JSON.stringify(e)}`);
    } else {
      let after = new Date();
      log.silly(
        `[bot.js] getClock: request took ${
          after.getTime() - before.getTime()
        }ms, diff = ${resp.currentTime - after}`
      );
    }
  });
};
